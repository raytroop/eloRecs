...
import numpy as np
from utils import one_hot_encoder, reduce_mem_usage, timer
...
def train_test(num_rows=None, log_name=None):
    logger = logging.getLogger(log_name)
    # load csv
    train_df = pd.read_csv('dataset/train.csv', index_col=['card_id'], nrows=num_rows)
    test_df = pd.read_csv('dataset/test.csv', index_col=['card_id'], nrows=num_rows)

    logger.info("Train samples: {}, test samples: {}".format(len(train_df), len(test_df)))
...
def historical_transactions(num_rows=None):
    # load csv
    hist_df = pd.read_csv('dataset/historical_transactions.csv', nrows=num_rows)
...
def new_merchant_transactions(num_rows=None):
    # load csv
    new_merchant_df = pd.read_csv('dataset/new_merchant_transactions.csv', nrows=num_rows)
...
def get_df(debug=False, log_name=None):
    num_rows = 10000 if debug else None
    logger = logging.getLogger(log_name)

    logger.info("train & test")
    df = train_test(num_rows, log_name)

    logger.info("historical transactions")
    df = pd.merge(df, historical_transactions(num_rows), on='card_id', how='outer')

    logger.info("new merchants")
...
if __name__ == '__main__':
    ...
    import time
    set_logger('optuna_lgbm', log_path='logs/logs_' +time.strftime("%m%d-%H%M%S"))
    train_df, test_df = get_df(log_name='optuna_lgbm')
